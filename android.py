...
def setup_pgo(context):
    """Set up Profile Guided Optimization environment"""
    if not hasattr(context, 'pgo') or not context.pgo:
        return
    ...
    run(["make", "profile-gen"], env=env)
    run(["make", "profile-run"], env=env)
    run(["make", "profile-merge"], env=env)
    
    # Use profile data
    run(["make", "profile-use"], env=env)
